...
/**
 * Created by wongxd on 2017/10/19 0019.
 *
 */
...
//                    ToastUtil.cT(context, context.getPackageName() + "--" + appProcess.processName + "处于前台");
     ...
     * @param tel 电话号码
     */
    public static void callPhone(Context ctx, String tel) {
        // 拨号：激活系统的拨号组件
        Intent intent = new Intent(); // 意图对象：动作 + 数据
        intent.setAction(Intent.ACTION_CALL); // 设置动作
        intent.setData(Uri.parse("tel:" + tel)); // 设置数据)
        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        ctx.startActivity(intent); // 激活Activity组件
    }
     ...
     * @param ctx
     */
    public static void intoAppSetting(Context ctx) {
